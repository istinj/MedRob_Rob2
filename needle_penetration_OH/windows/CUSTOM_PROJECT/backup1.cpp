...
hduVector3Dd needle_velocity(0.0f, 0.0f, 0.0f);
...
HDdouble needle_PENETRATION[2] = (0.0f,0.0f,0.0f);
HDdouble height[3] = (0.0f,1.0f,2.0f,3.0f); //--
...
HDCallbackCode HDCALLBACK hdEndCB(void *data)
{
	...
	//Prendo la velocità corrente
	hdGetDoublev(HD_CURRENT_VELOCITY, needle_velocity);
	...
	if(is_touched)
	{
		...
		//force_trans.normalize();
		 ...
		 needle_DOP = device_contact_pos[1] - device_pos[1]; //-- VALIDO SSE SUPERFICIE ORIZZONTALE NON RUOTATA CON NORMALE VERSO L'ALTO
		 ...
		 {
		 	is_touched = false;
		 	needle_PENETRATION[0] = 0;
		 	cout << "***************\nNow outiside\n***************" << endl;
		 }

		 needle_vector = device_pos - device_contact_pos; //NON SO SE è MEGLIO CALCOLARLA COSì O CONSIDERARLA SEMPRE LUNGO LA SNAP LINE (IDEALE!)


		if(needle_DOP < height[0])
		{
		needle_PENETRATION[0] = sqrt(needle_vector[0]*needle_vector[0] + 
			needle_vector[1]*needle_vector[1] + 
			needle_vector[2]*needle_vector[2]);
		}
		else if(needle_DOP >= height[0] && needle_DOP <= height[1])
		{
			needle_PENETRATION[0]=sqrt(height[0]*height[0]+PROXYPOINT*PROXYPOINT); //-- COME DERIVI PROXYPOINT?
			needle_PENETRATION[1]=il resto //-- che casino.. continua così anche per gli altri livelli.......
		}
		else if(needle_DOP >= height[1] && needle_DOP <= height[2])
		{

		}
		else if(needle_DOP >= height[2] && needle_DOP <= height[3])
		{

		}


		//CALCOLA LA MAGNITUDINE DELLA FORZA IN BASE ALLA NEEDLE PENETRATION
		double force_mag = 0.0f;
		if(needle_DOP >= height[0] && needle_DOP < height[1])
		{
			force_mag = k[0] * needle_PENETRATION[0];
			if(force_mag > max_mag_force[0])
			{
				force_mag = b[0] * needle_PENETRATION[0] * needle_velocity;
			}
		}
		else if(needle_DOP >= height[1] &&  needle_DOP< height[2])
		{
			force_mag = k[1] * needle_PENETRATION[1] + (b[0] * needle_PENETRATION[0] * needle_velocity);
			if(force_mag > max_mag_force[1])
			{
				force_mag = (b[0] * needle_PENETRATION[0] + b[1] * needle_PENETRATION[1]) * needle_velocity;
			}
		}
		else if(needle_DOP >= height[2] &&  needle_DOP< height[3])
		{
			force_mag = ;
			if(force_mag > max_mag_force[2])
			{
				force_mag = ;
			}
		}
		else if(needle_DOP >= height[3] &&  needle_DOP< height[4])
		{
			force_mag = ;
			if(force_mag > max_mag_force[3])
			{
				force_mag = ;
			}
		}
		...
		auto total_needle_penetration = needle_PENETRATION[0]+needle_PENETRATION[1]+needle_PENETRATION[2]+needle_PENETRATION[3];
		force = needle_vector/total_needle_penetration;
		force = force_mag * force;
		
		
		// force.normalize();
		hdSetDoublev(HD_CURRENT_FORCE, force*1.0);
	}
