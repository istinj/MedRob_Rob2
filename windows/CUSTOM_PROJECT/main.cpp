...
HDCallbackCode HDCALLBACK hdEndCB(void *data)
{
	...
	if(is_touched){
		...
		//Calcola la DOP
		needle_DOP = device_contact_pos[1] - device_pos[1]; //! valido sse superficie orizzontale


		//Calcola il needle_vector
		needle_vector = device_pos - device_contact_pos; //! sempre calcolato rispetto alla posizione reale del tip e non a quella ideale (ideale=sulla linea constraint)
